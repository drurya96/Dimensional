...
TEST_F(SimplificationTest, SimplificationWithMath) {
   ...
   ASSERT_TRUE((is_same<decltype(test), BaseDimension<TimeUnit<>>>::value));
...
TEST_F(SimplificationTest, Simplification) {
   ...
   auto test_before = BaseDimension<LengthUnit<>, TimeUnit<>, TimeUnit<Inverse>, LengthUnit<Inverse>, TimeUnit<Inverse>>(10.0, vector<BaseUnit<>*>{&Meters, &Minutes}, vector<BaseUnit<>*>{&Feet, &Seconds, &Minutes});
   ASSERT_TRUE((is_same<decltype(test_before), BaseDimension<LengthUnit<>, TimeUnit<>, TimeUnit<Inverse>, LengthUnit<Inverse>, TimeUnit<Inverse>>>::value));
   ...
   ASSERT_TRUE((is_same<decltype(test_after), BaseDimension<TimeUnit<Inverse>>>::value));
