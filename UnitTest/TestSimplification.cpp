...
TEST_F(SimplificationTest, SimplificationWithMath) {
   ...
   ASSERT_DOUBLE_EQ(test.GetVal(UnitList(TimeUnits::Seconds()), UnitList()), 0.056959027777777775); // TODO: Double check precision
}
...
TEST_F(SimplificationTest, Simplification) {
   ...
   auto test_before = BaseDimension(10.0, UnitList(Meters(), Minutes()), UnitList(Seconds(), Feet(), Minutes()));
   ...
   ASSERT_DOUBLE_EQ(test_after.GetVal(UnitList(), UnitList(TimeUnits::Minutes())), 1968.504);
}
