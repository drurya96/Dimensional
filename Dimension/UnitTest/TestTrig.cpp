...
TEST_F(TrigTest, AcosTest) {
   ...
   ASSERT_NEAR(Dimension::acos(-1.0).GetAngle<Radians>(), pi, TOLERANCE);

   ASSERT_NEAR(Dimension::acos(std::sqrt(3.0) / 2.0).GetAngle<Radians>(), pi / 6.0, TOLERANCE);
...
TEST_F(TrigTest, AsinTest) {
   ...
   ASSERT_NEAR(Dimension::asin(0.5).GetAngle<Radians>(), pi / 6.0, TOLERANCE);
...
TEST_F(TrigTest, AtanTest) {
   ...
   ASSERT_NEAR(Dimension::atan(std::sqrt(3.0)).GetAngle<Radians>(), pi / 3.0, TOLERANCE);
...
TEST_F(TrigTest, Atan2Test) {
   ...
   ASSERT_NEAR(Dimension::atan2(Length<Meters>(-1.0), Length<Meters>(0.0)).GetAngle<Radians>(), -pi / 2.0, TOLERANCE);
   ASSERT_NEAR(Dimension::atan2(Length<Meters>(0.0), Length<Meters>(-1.0)).GetAngle<Radians>(), pi, TOLERANCE);
   ASSERT_NEAR(Dimension::atan2(Length<Meters>(1.0), Length<Meters>(1.0)).GetAngle<Radians>(), pi / 4.0, TOLERANCE);
   ASSERT_NEAR(Dimension::atan2(Length<Meters>(-1.0), Length<Meters>(-1.0)).GetAngle<Radians>(), -3.0 * pi / 4.0, TOLERANCE);
