...
namespace dimension
{
   ...
   concept IsNamedtorqueUnit = requires {
      ...
      requires 
         ...
         is_angle_unit<typename std::tuple_element_t<3, typename T::units>::unit>;
      requires !std::is_base_of_v<FundamentalUnitTag, T>;
   };

   /// @brief Concept to verify a dimension can be treated as a torque type
   template<typename T, typename Rep>
   concept is_torque_as = dimension_convertible_to<T, Rep,
   ...
   template<typename T>
   concept is_torque = is_torque_as<T, double>;
   ...
   /// @tparam timespanUnit The timespan unit used for all timespan components of torque
   ...
   [[maybe_unused]]
   constexpr DimType::rep get_torque_as(const DimType& obj)
   {
   ...
   template<
      ...
      is_coefficient... Cs
   >
   requires are_torque_units<
      T0,
      T1,
      T2,
      T3
   >
   class torque<T0, T1, T2, T3, Cs...> : public base_dimension_impl<double,
      ...
      unit_exponent<typename unit_filter<angleType, T0, T1, T2, T3>::type, -1>,
      Cs...
   >
   {
   public:
   ...
   requires are_torque_units<
      ...
      T3
   >
   class torque<Rep, T0, T1, T2, T3, Cs...> : public base_dimension_impl<Rep,
      unit_exponent<typename unit_filter<massType, T0, T1, T2, T3>::type, 1>,
      unit_exponent<typename unit_filter<lengthType, T0, T1, T2, T3>::type, 2>,
      unit_exponent<typename unit_filter<timespanType, T0, T1, T2, T3>::type, -2>,
      unit_exponent<typename unit_filter<angleType, T0, T1, T2, T3>::type, -1>,
   ...
   template<IsNamedtorqueUnit Named, is_coefficient... Cs>
   class torque<Named, Cs...> : public base_dimension_from_tuple<double, typename Named::units, std::tuple<Cs...>>::dim
   {
   public:
   ...
   /// @brief Template specialization for named torque units
   /// @tparam Named The named unit this torque type is in terms of
   template<rep_type Rep, IsNamedtorqueUnit Named, is_coefficient... Cs>
   class torque<Rep, Named, Cs...> : public base_dimension_from_tuple<Rep, typename Named::units, std::tuple<Cs...>>::dim
   {
   public:
   ...
   // TODO: Unit test this and remove suppression
   [[maybe_unused]]
   constexpr auto make_torque(Cs... coeffs)
   {
      return torque<double, T0, T1, T2, T3, Cs...>(1.0, coeffs...);
   }
   ...
   // TODO: Unit test this and remove suppression
   [[maybe_unused]]
   constexpr auto make_torque(Rep value, Cs... coeffs)
   {
      return torque<Rep, T0, T1, T2, T3, Cs...>(value, coeffs...);
   }
   ...
   constexpr auto make_torque(Cs... coeffs)
   {
      return torque<double, Named, Cs...>(1.0, coeffs...);
   }

   /// @brief Template specialization for named torque units
   /// @tparam Named The named unit this torque type is in terms of
   template<IsNamedtorqueUnit Named, rep_type Rep, is_coefficient... Cs>
   // TODO: Unit test this and remove suppression
   [[maybe_unused]]
   constexpr auto make_torque(Rep value, Cs... coeffs)
   {
      return torque<Rep, Named, Cs...>(value, coeffs...);
   }
