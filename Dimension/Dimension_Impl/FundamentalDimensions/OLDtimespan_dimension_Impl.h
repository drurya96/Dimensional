...
namespace dimension
{
   ...
   public:
      ...
      // URGENT TODO: This also needs to use the coefficient types rather than get_dimension_as
   ...
   struct simplify_timespan_dim<base_dimension_impl<R, UExp, Cs...>> {
      ...
      using type = timespan<R, typename UExp::unit, Cs...>;
   };

   template<class Dim>
   using simplify_timespan_dim_t =
      typename simplify_timespan_dim<simplify_dimension_t<Dim>>::type;
   ...
   */
   ...
   timespan(Dim) -> simplify_timespan_dim_t<Dim>;
