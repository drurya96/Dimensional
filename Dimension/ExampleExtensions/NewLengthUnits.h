...
namespace dimension
{
   ...
   template<> struct Conversion<Feet, Fail> { static constexpr double slope = 3.14; };
   template<> struct Conversion<Fail, Feet> { static constexpr double slope = (1.0 / 3.14); };

   template<> struct Conversion<meters, Foo> { static constexpr double slope = 3.14; };
   template<> struct Conversion<Foo, meters> { static constexpr double slope = (1.0 / 3.14); };

   template<> struct Conversion<meters, Bar> { static constexpr double slope = 25.0; };
   template<> struct Conversion<Bar, meters> { static constexpr double slope = (1.0 / 25.0); };

   template<> struct Conversion<meters, Baz>
   {
      static constexpr double slope = (9.0/5.0);
      static constexpr double offset = 32.0;
   };

   template<> struct Conversion<Baz, meters> 
   { 
      static constexpr double slope = (5.0/9.0);
      static constexpr double offset = -17.777777778;
   };
